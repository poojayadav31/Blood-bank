  ...
  const handleGenerate = async (e) => {
    ...
    try {
      if (format === 'csv' || format === 'xlsx') {
        const url = `/api/reports?type=${reportType}&format=${format}&organization_id=${user?.organization_id || 1}`;
        window.open(url, '_blank');
      } else {
        const res = await request(`/reports?type=${reportType}&format=json&organization_id=${user?.organization_id || 1}`);
    ...
    <div style={{ padding: '30px' }}>
      ...
      <div style={{ marginBottom: '28px' }}>
        ...
        <p style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>
          Generate localized inventory, camp collection performance, and hospital dispatch reports for {user?.organization?.name || 'your organization'}.
        </p>
      </div>
        ...
        <form onSubmit={handleGenerate} style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', alignItems: 'flex-end' }}>
          <div style={{ flex: '1 1 240px' }}>
            ...
            <select className="form-select" value={reportType} onChange={(e) => setReportType(e.target.value)}>
              <option value="inventory">Blood Stock & Inventory Status</option>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            <button type="submit" disabled={loading} className="btn btn-primary" style={{ padding: '10px 22px' }}>
              ...
              {loading ? 'Generating...' : format === 'json' ? 'Preview Report' : `Export ${format.toUpperCase()}`}
            </button>
          </div>
        </form>
      </div>
        ...
        <div className="glass-card" style={{ padding: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button
                onClick={() => {
                  window.open(`/api/reports?type=${reportType}&format=csv&organization_id=${user?.organization_id || 1}`, '_blank');
                }}
              ...
              <button
                onClick={() => {
                  window.open(`/api/reports?type=${reportType}&format=xlsx&organization_id=${user?.organization_id || 1}`, '_blank');
                }}
                className="btn btn-secondary"
                style={{ padding: '6px 12px', fontSize: '0.8rem' }}
              >
